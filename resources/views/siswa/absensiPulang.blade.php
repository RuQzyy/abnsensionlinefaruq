...
@section('title', 'Absen Pulang Siswa')

@section('content')
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-4xl font-bold text-center text-gray-800 mb-8">Absensi Pulang Siswa</h1>
...
@push('scripts')
  <script>
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      try {
        const response = await fetch("{{ route('siswa.absensiPulang.store') }}", {
          method: 'POST',
          headers: {
        ...
        if (result.success) {
          ...
          window.location.href = "{{ route('siswa.index') }}"; // atau redirect ke halaman yang Anda inginkan
        } else {

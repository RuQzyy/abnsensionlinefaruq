  ...
  <div class="max-w-3xl mx-auto bg-white p-10 rounded-2xl shadow-lg mt-8">
    ...
    <form action="{{ route('guru.update', $guru->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $guru->name) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 transition">
        @error('name') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
      </div>
      <div>
        <label for="nip" class="block text-sm font-medium text-gray-700 mb-1">NIP</label>
        <input type="text" id="nip" name="nip" value="{{ old('nip', $guru->nip) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 transition">
